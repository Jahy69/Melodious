...
export default {
	...
	methods: {
		async SendBot(query) {
			...
			if (botResponse.playlists && botResponse.playlists.length > 0) {
				let playlists = botResponse.playlists;
				let playlistItems = '';
				for (let i = 0; i < playlists.length; i++) {
					playlistItems += `- ${playlists[i].name}: ${playlists[i].url}\n`;
				}
				answer += `\nVoici quelques playlists qui pourrait vous intéresser :\n${playlistItems}`;
		...
		<div class="content">
			<div class="demo-left">
				<div class="chatboxCont">
					...
					<div class="chatbox" id="chatbox">
						<div class="imessage">
							<div :class="[message.author == 'server' ? 'from-them' : 'from-me']"
								v-for="(message, idx) in chat_box_repertory" :key="idx">
					...
					<div class="inputBox">
						<input v-if="!loading" class="inputUser" type="text" @keyup.enter="SendBot(input)" v-model="input"
			...
			<div class="demo-right">
				...
				<div class="text">
					Notre chatbot intelligent utilise l'intelligence artificielle pour enrichir votre expérience musicale de
					manière interactive et éducative. Voici ce que vous pouvez faire avec Melodious : <br /><br />•
					Explorer des genres musicaux : De la pop au baroque, découvrez les subtilités et les histoires derrière divers
					genres musicaux.<br /><br />
					• Découvrir des artistes et des chansons : Trouvez de nouveaux favoris avec des recommandations personnalisées
					basées sur vos goûts.<br /><br />
					• Apprendre des techniques musicales : Obtenez des explications sur les techniques utilisées dans vos morceaux
					préférés.<br /><br />
					• Écouter des anecdotes musicales : Plongez dans l'histoire et les origines des chansons et des mouvements
					musicaux.<br /><br />
